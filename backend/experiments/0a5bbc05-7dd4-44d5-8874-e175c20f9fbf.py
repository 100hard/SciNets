...
# We'll compute, for each run, mean A before onsets vs random non-onset times; if many runs do not show A_onset > A_non_onset, label as noise-dominated
...
for e in ensemble_baseline:
    ...
    if len(onsets) == 0:
        continue
    # sample A at onsets and random wake times
    a_on = A[onsets]
    # sample equal number of random times from awake times
...
# We'll pick a representative run (median total sleep) and summarize
...
avg_accumulation = np.mean(np.diff(A_rep)[S_rep[:-1] == 0]) if np.any(S_rep[:-1] == 0) else np.nan
avg_clearance = np.mean(np.diff(A_rep)[S_rep[:-1] == 1]) if np.any(S_rep[:-1] == 1) else np.nan
...
# We'll compute fraction of onsets where A_onset >= theta (its run-specific theta)
onset_supports = []
for e in ensemble_baseline:
    p = e['params']
    out = e['out']
...
# Heuristic: if >75% of onsets occur at/above run-specific theta -> yes; 40-75% partial; else no
...
try:
    ...
    # inset: histogram of total sleep across ensemble
    plt.tight_layout()
