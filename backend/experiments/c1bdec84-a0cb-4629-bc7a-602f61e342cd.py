...
def run_one(seed, apply_l1=False):
    ...
    # OOD: we'll construct dataset where cue is predictive of opposite class
    X_tmp, y_tmp, _ = make_dataset(ood_size, cue_balance=0.9, cue_correlated_with_label=True, seed=seed+300)
    # flip labels to make cue predictive of opposite class by swapping labels
    y_test_ood = 1 - y_tmp
    ...
    for i in range(ood_size):
        ...
        intensity = 1.0 if label == 1 else -1.0
        img[cx - half:cx + half, cy - half:cy + half] += intensity
        # convert to torch tensor before assignment to avoid dtype mismatch
        X_test_ood[i, 0] = torch.from_numpy(img).float()
