...
# Small, simplified exploratory dynamical model
np.random.seed(0)
...
T = 50            # timesteps (<=50 to meet epoch constraint)
dt = 0.2
ensemble = 3      # small ensemble to keep total samples small
scenarios = {
...
for name, sc in scenarios.items():
    ...
    for i in range(ensemble):
        # small param jitter ±10%
        pert = {k: max(1e-6, v * rng.normal(1.0, 0.10)) for k,v in params.items()}
...
# sensitivity: vary two params ±25%
